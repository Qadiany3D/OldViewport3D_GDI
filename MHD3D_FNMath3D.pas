...
type
    ...
    public
        ...
        function Convert(X,Y,Amount : Extended):Extended;
        ...
        function VMul(const V:TVec3D;const M:TMatrix3D;const WithoutRow4: boolean):TVec3D;overload;
        ...
        function GetRotOfMtrx(const M : TMatrix3D):TVec3D;
    end;
...
procedure TFNMath3D.SetMaxVal(L:extended);
begin
    l := abs(l);
   if (l > 1.1e+4900) then
        l := 1.1e+4900
...
procedure TFNMath3D.SetMinVal(L:extended);
begin
    l := abs(l);
    if l < 3.4e-4900 then
        l := 3.4e-4900
...
function TFNMath3D.GetMaxVal:extended;
begin
    result := self.maximumvalue;
end;

function TFNMath3D.GetMinVal:extended;
begin
    result := self.minimumvalue;
end;

constructor TFNMath3D.Create(MinValue: extended = 1.1e-30;MaxValue: extended = 1.1e+30);
begin
    self.MaxVal := MaxValue;
    self.MinVal := MinValue;
end;

function TFNMath3D.Convert(X,Y,Amount : Extended):Extended;
begin
    result := x - (x - y) * amount;
end;

function TFNMath3D.TimeBet(const T1,T2:TDateTime):String;
var
    x,y,z,w : integer;
begin
...
function TFNMath3D.StrFromFloat(const x:extended;const digit:integer):string;
var
    i:integer;
    s:string;
begin
...
function TFNMath3D.Limit(const x,a,b:extended):extended;
begin
    if a < b then
    begin
         if x < a then result := a
...
function TFNMath3D.ColorWave1(Color:extended = 0;White:extended = 0;black:extended = 0):tcolor;
const
    p_6 = 0.523598775598298873077107230546583814;
    p2 = 6.2831853071795864769252867665590057682;
    p_3 = 1.047197551196597746154214461093167628;
...
function TFNMath3D.ColorWave2(Color:extended = 0;White:extended = 0;black:extended = 0):tcolor;
const
    p_6 = 0.523598775598298873077107230546583814;
    p2 = 6.2831853071795864769252867665590057682;
    p_3 = 1.047197551196597746154214461093167628;
...
function TFNMath3D.MonoColor(c:TColor):TColor;
var
    x:integer;
begin
    x := (getrvalue(c) + getgvalue(c) + getbvalue(c)) div 3;
...
function TFNMath3D.Vec2D(const X,Y:extended):TVec2D;
begin
    result.X := x;
    result.Y := y;
end;

function TFNMath3D.Vec3D(const X,Y,Z:extended):TVec3D;
begin
    result.X := x;
    result.Y := y;
    result.Z := z;
...
function TFNMath3D.Vec4D(const X,Y,Z,w:extended):TVec4D;
begin
    result.X := x;
    result.Y := y;
    result.Z := z;
...
function TFNMath3D.Ray3D(const XPos,YPos,ZPos ,
...
function TFNMath3D.Ray3D(const Pos,Dir:TVec3D):TRay3D;
begin
    result.Pos := pos;
    result.Dir := self.Norm(dir);
end;

function TFNMath3D.Matrix3D(const X1,Y1,Z1 ,
                              X2,Y2,Z2 ,
                              X3,Y3,Z3 ,
...
function TFNMath3D.Matrix3D(const V1,V2,V3,V4:TVec3D;DoNormal: boolean = true):TMatrix3D;
begin
    if DoNormal then
    begin
...
function TFNMath3D.Matrix3D(const L:extended):TMatrix3D;
begin
    with result do
    begin
        _11 := 1; _12 := 0; _13 := 0;
...
function TFNMath3D.AsStr(const V:TVec2D;const Digit:integer=17):string;
begin
    result := '[' +
...
function TFNMath3D.AsStr(const V:TVec3D;const Digit:integer=17):string;
begin
    result := '[' +
          self.StrFromFloat(v.X,digit) +
          ', ' +
...
function TFNMath3D.AsStr(const V:TVec4D;const Digit:integer=17):string;
begin
    result := '[' +
          self.StrFromFloat(v.X,digit) +
          ', ' +
...
function TFNMath3D.AsStr(const R:TRay3D;const Digit:integer=17):string;
begin
    result := '(Ray3D ' +
...
function TFNMath3D.AsStr(const M:TMatrix3D;const Digit:integer=17):string;
begin
    result := '(Matrix3D ' +
...
function TFNMath3D.Equal(const X,Y:extended;const Limit:extended=0.000001):boolean;
begin
    result := abs(x-y) < Limit;
end;

function TFNMath3D.Equal(const V1,V2:TVec2D;const Limit:extended=0.000001):boolean;
begin
    result := (abs(v1.X - v2.x) < Limit) and (abs(v1.y - v2.Y) < Limit);
end;

function TFNMath3D.Equal(const V1,V2:TVec3D;const Limit:extended=0.000001):boolean;
begin
    result := (abs(v1.X - v2.X) < Limit) and (abs(v1.y - v2.Y) < Limit) and (abs(v1.z - v2.z) < Limit);
end;

function TFNMath3D.Equal(const V1,V2:TVec4D;const Limit:extended=0.000001):boolean;
begin
    result := (abs(v1.X - v2.X) < Limit) and (abs(v1.y - v2.Y) < Limit) and (abs(v1.z - v2.z) < Limit) and (abs(v1.w - v2.w) < Limit);
end;

function TFNMath3D.Equal(const R1,R2:TRay3D;const Limit:extended=0.000001):boolean;
begin
    result := self.Equal(r1.Pos,r2.Pos,limit) and self.Equal(r1.Dir,r2.Dir,limit);
end;

function TFNMath3D.Equal(const M1,M2:TMatrix3D;const Limit:extended=0.000001):boolean;
begin
...
function TFNMath3D.GetRow(const V:TVec2D;const i:byte):extended;
begin
    result := v._[i];
end;

function TFNMath3D.GetRow(const V:TVec3D;const i:byte):extended;
begin
    result := v._[i];
end;

function TFNMath3D.GetRow(const V:TVec4D;const i:byte):extended;
begin
    result := v._[i];
end;

function TFNMath3D.GetRow(const R:TRay3D;const i:byte):TVec3D;
begin
    if (i = 2) then result := self.Norm(r.Dir)
    else result := r.Pos;
end;

function TFNMath3D.GetRow(const R:TRay3D;const i,j:byte):extended;
begin
...
function TFNMath3D.GetRow(const M:TMatrix3D;const i:byte):TVec3D;
begin
...
function TFNMath3D.GetRow(const M:TMatrix3D;const i,j:byte):extended;
begin
...
procedure TFNMath3D.SetRow(var V:TVec2D;const i:byte; L:extended);
begin
    if (l < 0) then
    begin
...
procedure TFNMath3D.SetRow(var V:TVec3D;const i:byte; L:extended);
begin
    if (l < 0) then
    begin
        if (-l > self.MaximumValue) then v._[i] := -self.MaximumValue
...
procedure TFNMath3D.SetRow(var V:TVec4D;const i:byte; L:extended);
begin
    if (l < 0) then
    begin
        if (-l > self.MaximumValue) then v._[i] := -self.MaximumValue
...
procedure TFNMath3D.SetRow(var R:TRay3D;const i:byte; V:TVec3D);
var
  k: Integer;
begin
    for k := 1 to 3 do
...
procedure TFNMath3D.SetRow(var R:TRay3D;const i,j:byte; L:extended);
begin
    if (l < 0) then
    begin
...
procedure TFNMath3D.SetRow(var M:TMatrix3D;const i:byte; V:TVec3D);
var
    k: byte;
begin
    for k := 1 to 3 do
...
procedure TFNMath3D.SetRow(var M:TMatrix3D;const i,j:byte; L:extended);
begin
    if (l < 0) then
    begin
...
procedure TFNMath3D.SetRow(var M:TMatrix3D;const i:byte; X,Y,Z:extended);
begin
    if (x < 0) then
    begin
...
function TFNMath3D.VConvert(const V1,V2:TVec2D;L: extended):TVec2D;
begin
    l := self.Limit(l,-1,1);
...
function TFNMath3D.VConvert(const V1,V2:TVec3D;L: extended):TVec3D;
begin
    l := self.Limit(l,-1,1);
    result.X := (v2.X - v1.X) * l + v1.X;
    result.Y := (v2.Y - v1.Y) * l + v1.y;
...
function TFNMath3D.VConvert(const V1,V2:TVec4D;L: extended):TVec4D;
begin
    l := self.Limit(l,-1,1);
    result.X := (v2.X - v1.X) * l + v1.X;
    result.Y := (v2.Y - v1.Y) * l + v1.y;
...
function TFNMath3D.VAdd(const V1,V2:TVec2D):TVec2D;
begin
    result.X := v1.X + v2.X;
    result.Y := v1.Y + v2.Y;
end;
...
function TFNMath3D.VAdd(const V1,V2:TVec3D):TVec3D;
begin
    result.X := v1.X + v2.X;
    result.Y := v1.Y + v2.Y;
    result.Z := v1.Z + v2.Z;
...
function TFNMath3D.VAdd(const V1,V2:TVec4D):TVec4D;
begin
    result.X := v1.X + v2.X;
    result.Y := v1.Y + v2.Y;
    result.Z := v1.Z + v2.Z;
...
function TFNMath3D.VSum(const AV:array of TVec2D):TVec2D;
var
  I: Integer;
begin
    result.X := av[0].X;
...
function TFNMath3D.VSum(const AV:array of TVec3D):TVec3D;
var
  I: Integer;
begin
    result.X := av[0].X;
...
function TFNMath3D.VSum(const AV:array of TVec4D):TVec4D;
var
  I: Integer;
begin
    result.X := av[0].X;
...
function TFNMath3D.VSub(const V1,V2:TVec2D):TVec2D;
begin
    result.X := v1.X - v2.X;
    result.Y := v1.Y - v2.Y;
end;

function TFNMath3D.VSub(const V1,V2:TVec3D):TVec3D;
begin
    result.X := v1.X - v2.X;
    result.Y := v1.Y - v2.Y;
    result.Z := v1.Z - v2.Z;
...
function TFNMath3D.VSub(const V1,V2:TVec4D):TVec4D;
begin
    result.X := v1.X - v2.X;
    result.Y := v1.Y - v2.Y;
    result.Z := v1.Z - v2.Z;
...
function TFNMath3D.VMul(const V1,V2:TVec2D):TVec2D;
begin
    result.X := v1.X * v2.X;
    result.Y := v1.Y * v2.Y;
end;

function TFNMath3D.VMul(const V1,V2:TVec3D):TVec3D;
begin
    result.X := v1.X * v2.X;
    result.Y := v1.Y * v2.Y;
    result.Z := v1.Z * v2.Z;
...
function TFNMath3D.VMul(const V1,V2:TVec4D):TVec4D;
begin
    result.X := v1.X * v2.X;
    result.Y := v1.Y * v2.Y;
    result.Z := v1.Z * v2.Z;
...
function TFNMath3D.VMul(const L:extended;const V:TVec2D):TVec2D;
begin
    result.X := v.X * l;
    result.Y := v.Y * l;
end;

function TFNMath3D.VMul(const L:extended;const V:TVec3D):TVec3D;
begin
    result.X := v.X * l;
    result.Y := v.Y * l;
    result.Z := v.Z * l;
...
function TFNMath3D.VMul(const L:extended;const V:TVec4D):TVec4D;
begin
    result.X := v.X * l;
    result.Y := v.Y * l;
    result.Z := v.Z * l;
...
function TFNMath3D.VMul(const V:TVec2D;const L:extended):TVec2D;
begin
    result.X := v.X * l;
    result.Y := v.Y * l;
end;

function TFNMath3D.VMul(const V:TVec3D;const L:extended):TVec3D;
begin
    result.X := v.X * l;
    result.Y := v.Y * l;
    result.Z := v.Z * l;
...
function TFNMath3D.VMul(const V:TVec4D;const L:extended):TVec4D;
begin
    result.X := v.X * l;
    result.Y := v.Y * l;
    result.Z := v.Z * l;
...
function TFNMath3D.VMul(const V:TVec3D;const M:TMatrix3D;const WithoutRow4: boolean):TVec3D;
begin
     result.X := v.X * m.Row1.X + v.Y * m.Row2.X + v.Z * m.Row3.X;
     result.Y := v.X * m.Row1.Y + v.Y * m.Row2.Y + v.Z * m.Row3.Y;
     result.Z := v.X * m.Row1.Z + v.Y * m.Row2.Z + v.Z * m.Row3.Z;
end;

function TFNMath3D.VMul(const V:TVec3D;const M:TMatrix3D):TVec3D;
begin
...
function TFNMath3D.VDiv(const V1,V2:TVec2D):TVec2D;
{const
...
function TFNMath3D.VDiv(const V1,V2:TVec3D):TVec3D;
{const
    inf_ = 0.00000000000000000001;}
begin
    if v2.X <> 0 then result.X := v1.X / v2.X else result.X := v1.X / self.MinVal;
...
function TFNMath3D.VDiv(const V1,V2:TVec4D):TVec4D;
{const
    inf_ = 0.00000000000000000001;}
begin
    if v2.X <> 0 then result.X := v1.X / v2.X else result.X := v1.X / self.MinVal;
...
function TFNMath3D.VDiv(const V:TVec2D;L:extended):TVec2D;
begin
...
function TFNMath3D.VDiv(const V:TVec3D;L:extended):TVec3D;
begin
    if l = 0 then l := self.MinVal;//0.00000000000000000001;
    result.X := v.X / l;
    result.y := v.y / l;
...
function TFNMath3D.VDiv(const V:TVec4D;L:extended):TVec4D;
begin
    if l = 0 then l := self.MinVal;//0.00000000000000000001;
    result.X := v.X / l;
    result.y := v.y / l;
...
function TFNMath3D.VNeg(const V:TVec2D):TVec2D;
begin
    result.X := -v.X;
    result.Y := -v.Y;
end;

function TFNMath3D.VNeg(const V:TVec3D):TVec3D;
begin
    result.X := -v.X;
    result.Y := -v.Y;
    result.z := -v.z;
...
function TFNMath3D.VNeg(const V:TVec4D):TVec4D;
begin
    result.X := -v.X;
    result.Y := -v.Y;
    result.z := -v.z;
...
function TFNMath3D.VRev(const V:TVec2D):TVec2D;
{const
...
function TFNMath3D.VRev(const V:TVec3D):TVec3D;
{const
    inf_ = 10000000000000000000.0;}
begin
    if v.X <> 0 then result.X := 1 / v.X else result.X := self.MaxVal;
...
function TFNMath3D.VRev(const V:TVec4D):TVec4D;
{const
    inf_ = 10000000000000000000.0;}
begin
    if v.X <> 0 then result.X := 1 / v.X else result.X := self.MaxVal;
...
function TFNMath3D.VAbs(const V:TVec2D):TVec2D;
begin
    result.X := abs(v.X);
    result.y := abs(v.y);
end;

function TFNMath3D.VAbs(const V:TVec3D):TVec3D;
begin
    result.X := abs(v.X);
    result.y := abs(v.y);
    result.z := abs(v.z);
...
function TFNMath3D.VAbs(const V:TVec4D):TVec4D;
begin
    result.X := abs(v.X);
    result.y := abs(v.y);
    result.z := abs(v.z);
...
function TFNMath3D.Leng(const V:TVec2D):extended;
{begin
    result := sqrt(sqr(v.X) +
                   sqr(v.Y));
end;}
...
function TFNMath3D.Leng(const V:TVec3D):extended;
{begin
    result := sqrt(sqr(v.X) +
                   sqr(v.Y) +
                   sqr(v.Z));
...
function TFNMath3D.Leng(const V:TVec4D):extended;
{begin
    result := sqrt(sqr(v.X) +
                   sqr(v.Y) +
                   sqr(v.Z) +
...
function TFNMath3D.Leng(const x,y:extended):extended;
asm
    fld   x
    fmul st,st
    fld   y
...
function TFNMath3D.Leng(const x,y,z:extended):extended;
{begin
    result := sqrt(sqr(X) +
                   sqr(Y) +
                   sqr(Z));
...
function TFNMath3D.Leng(const x,y,z,w:extended):extended;
{begin
    result := sqrt(sqr(X) +
                   sqr(Y) +
                   sqr(Z) +
...
function TFNMath3D.Dist(const V1,V2:TVec2D):extended;
{begin
...
function TFNMath3D.Dist(const V1,V2:TVec3D):extended;
{begin
    result := sqrt(sqr(v1.X - v2.X) +
                   sqr(v1.Y - v2.Y) +
                   sqr(v1.z - v2.Z));
...
function TFNMath3D.Dist(const V1,V2:TVec4D):extended;
{begin
    result := sqrt(sqr(v1.X - v2.X) +
                   sqr(v1.Y - v2.Y) +
                   sqr(v1.z - v2.Z) +
...
function TFNMath3D.Dist(const M1,M2:TMatrix3D):extended;
begin
...
function TFNMath3D.Norm(const V:TVec2D):TVec2D;
var
    l:extended;
begin
    l := sqrt(sqr(v.X) +
...
function TFNMath3D.Norm(const V:TVec3D):TVec3D;
var
    l:extended;
begin
    l := sqrt(sqr(v.X) +
...
function TFNMath3D.Norm(const V:TVec4D):TVec4D;
var
    l:extended;
begin
    l := sqrt(sqr(v.X) +
...
function TFNMath3D.Norm(const x,y:extended):TVec2D;
var
    l:extended;
begin
    l := sqrt(sqr(X) +
...
function TFNMath3D.Norm(const x,y,z:extended):TVec3D;
var
    l:extended;
begin
    l := sqrt(sqr(X) +
...
function TFNMath3D.Norm(const x,y,z,w:extended):TVec4D;
var
    l:extended;
begin
    l := sqrt(sqr(X) +
...
function TFNMath3D.Norm(const R:TRay3D):TRay3D;
begin
    result.Pos := r.Pos;
    result.Dir := self.Norm(r.Dir);
end;

function TFNMath3D.Norm(const M:TMatrix3D):TMatrix3D;
begin
...
procedure TFNMath3D.SetLeng(var V:TVec2d;L:extended);
var
    l_:extended;
begin
...
procedure TFNMath3D.SetLeng(var V:TVec3d;L:extended);
var
    l_:extended;
begin
    if l = 0 then l := self.MinVal;//0.0000000000000000000001;
...
procedure TFNMath3D.SetLeng(var V:TVec4d;L:extended);
var
    l_:extended;
begin
    if l = 0 then l := self.MinVal;//0.0000000000000000000001;
...
function TFNMath3D.Dot(const V1,V2:TVec2D):extended;
{begin
    result := v1.X * v2.X +
              v1.Y * v2.Y;
end;}
...
function TFNMath3D.Dot(const V1,V2:TVec3D):extended;
asm
    fld     v1.x
    fld     v2.x
    fmul
...
function TFNMath3D.Dot(const V1,V2:TVec4D):extended;
{begin
    result := v1.X * v2.X +
              v1.Y * v2.Y +
              v1.Z * v2.Z +
...
function TFNMath3D.Cross(const V1,V2:TVec3D):TVec3D;
begin
...
function TFNMath3D.RotX(const M:TMatrix3D;const Angle:extended;const ApplyToRow4:boolean):TMatrix3D;
var
    Sin_,Cos_:extended;
begin
...
function TFNMath3D.RotY(const M:TMatrix3D;const Angle:extended;const ApplyToRow4:boolean):TMatrix3D;
var
    Sin_,Cos_:extended;
begin
    sincos(angle,sin_,cos_);
...
function TFNMath3D.RotZ(const M:TMatrix3D;const Angle:extended;const ApplyToRow4:boolean):TMatrix3D;
var
    Sin_,Cos_:extended;
begin
    sincos(angle,sin_,cos_);
...
function TFNMath3D.Rot(const M:TMatrix3D;const X_Angle,Y_Angle,Z_Angle:extended;const ApplyToRow4:boolean = true;const XYZ_Not_ZYX:boolean = true):TMatrix3D;
{
...
function TFNMath3D.PreRotX(M:TMatrix3D;const Angle:extended;const ApplyToRow4:boolean):TMatrix3D;
var
    Sin_,Cos_:extended;
...
begin    {
    ...
    BEGIN
        _11 := 1 ; _12 := 0 ; _13 := 0;
        _21 := 0 ; _22 := COS_; _23 := SIN_;
        _31 := 0 ; _32 := -SIN_; _33 := COS_;
        _41 := 0; _42 := 0;_43 := 0;
    END;
    //RESULT := SELF.MMul(M_,M);
    WITH RESULT DO
    BEGIN
        _11 := M_._11 * M._11 + M_._12 * M._21 + M_._13 * M._31;
        _12 := M_._11 * M._12 + M_._12 * M._22 + M_._13 * M._32;
        _13 := M_._11 * M._13 + M_._12 * M._23 + M_._13 * M._33;
        _21 := M_._21 * M._11 + M_._22 * M._21 + M_._23 * M._31;
        _22 := M_._21 * M._12 + M_._22 * M._22 + M_._23 * M._32;
        _23 := M_._21 * M._13 + M_._22 * M._23 + M_._23 * M._33;
        _31 := M_._31 * M._11 + M_._32 * M._21 + M_._33 * M._31;
        _32 := M_._31 * M._12 + M_._32 * M._22 + M_._33 * M._32;
        _33 := M_._31 * M._13 + M_._32 * M._23 + M_._33 * M._33;
        if ApplyToRow4 then
        BEGIN
            ROW4 := SELF.VAdd(self.VMul((1 - cos_) * self.dot(m.row1,m.row4) ,m.row1) ,
                              self.VMul(sin_,self.Cross(m.Row1,m.Row4))  );
            ROW4 := SELF.VAdd(ROW4,self.VMul(cos_,m.Row4));
        END
        ELSE
        BEGIN
            _41 := M._41;
...
function TFNMath3D.PreRotY(M:TMatrix3D;const Angle:extended;const ApplyToRow4:boolean):TMatrix3D;
var
    Sin_,Cos_:extended;
    //xx,yy,zz,Lengs_ :TVec3D;
    M_ : TMatrix3D;
begin    {
    ...
    WITH M_ DO
    BEGIN
        _11 := COS_ ; _12 := 0     ; _13 := -SIN_;
        _21 := 0    ; _22 := 1     ; _23 := 0    ;
        _31 := SIN_ ; _32 := 0     ; _33 := COS_;
        _41 := 0    ; _42 := 0     ;_43 := 0;
    END;
    //RESULT := SELF.MMul(M_,M);
    WITH RESULT DO
    BEGIN
        ...
        if ApplyToRow4 then
        BEGIN
            ROW4 := SELF.VAdd(self.VMul((cos_ - 1) * self.dot(m.row2,m.row4) ,m.row2) ,
                              self.VMul(-sin_,self.Cross(m.Row2,m.Row4))  );
            ROW4 := SELF.VAdd(ROW4,self.VMul(cos_,m.Row4));
        END
        ELSE
        BEGIN
...
function TFNMath3D.PreRotZ(M:TMatrix3D;const Angle:extended;const ApplyToRow4:boolean):TMatrix3D;
var
    Sin_,Cos_:extended;
    //xx,yy,zz,Lengs_ :TVec3D;
    M_: TMatrix3D;
begin    {
    ...
    WITH M_ DO
    BEGIN
        _11 := COS_  ; _12 := SIN_  ; _13 := 0 ;
        _21 := -SIN_ ; _22 := COS_  ; _23 := 0 ;
        _31 := 0     ; _32 := 0     ; _33 := 1 ;
        _41 := 0     ; _42 := 0     ; _43 := 0 ;
    END;
    //RESULT := SELF.MMul(M_,M);
    ...
    BEGIN
        ...
        if ApplyToRow4 then
        BEGIN
            ROW4 := SELF.VAdd(self.VMul((1 - cos_) * self.dot(m.row3,m.row4) ,m.row3) ,
                              self.VMul(-sin_,self.Cross(m.Row3,m.Row4))  );
            ROW4 := SELF.VAdd(ROW4,self.VMul(cos_,m.Row4));
        END
        ELSE
        BEGIN
...
function TFNMath3D.MAdd(const M1,M2:TMatrix3D):TMatrix3D;
begin
...
function TFNMath3D.MSub(const M1,M2:TMatrix3D):TMatrix3D;
begin
    result._11 := m1._11 - m2._11;
    result._12 := m1._12 - m2._12;
    result._13 := m1._13 - m2._13;
...
function TFNMath3D.MMul(const M1,M2:TMatrix3D):TMatrix3D;
var
    i,j: integer;
begin
...
function TFNMath3D.MMul(const M1,M2:TMatrix3D;const Row4FromM2:boolean):TMatrix3D;
var
  I,J: Integer;
begin
    result._11 := m1._11 * m2._11 + m1._12 * m2._21 + m1._13 * m2._31;
...
function TFNMath3D.Inverse(const M:TMatrix3D):TMatrix3D;
var
    det_: extended;
begin
    with m do
...
function TFNMath3D.MovD(const M:TMatrix3d;const Dir:TVec3D;const L:extended):TMatrix3D;
begin
    result := m;
    result.Row4 := self.VAdd(m.Row4,self.VMul(l,self.Norm(Dir)));
end;

function TFNMath3D.MovX(const M:TMatrix3d;const L:extended):TMatrix3D;
begin
    result := m;
    result.Row4 := self.VAdd(m.Row4,self.VMul(l,self.Norm(m.Row1)));
end;

function TFNMath3D.MovY(const M:TMatrix3d;const L:extended):TMatrix3D;
begin
    result := m;
    result.Row4 := self.VAdd(m.Row4,self.VMul(l,self.Norm(m.Row2)));
end;

function TFNMath3D.MovZ(const M:TMatrix3d;const L:extended):TMatrix3D;
begin
    result := m;
    result.Row4 := self.VAdd(m.Row4,self.VMul(l,self.Norm(m.Row3)));
end;

function TFNMath3D.TransWith(const M,TransferM:TMatrix3D):TMatrix3D;
begin
    Self.Matrix3D(1);
end;

function TFNMath3D.TransWith(const V:TVec3D;const TransferM:TMatrix3D):TVec3D;
begin
...
function TFNMath3D.V3DToScreen2(const V:TVec3D;const M:TMatrix3D;const FOV:extended;const Radius,CenterX,CenterY:integer):tpoint;
var
...
function TFNMath3D.V3DToScreen(const V:TVec3D;const M:TMatrix3D;const FOV:extended;const Radius,CenterX,CenterY:integer):tpoint;
var
    v_ : tvec3d;
    //m_ : TMatrix3D;
    x,y,z:Extended;
begin
    ...
    {
    v_.Z := v_.Z * tan(FOV / 2.0) * 2.0;
    v_.Z := Min(v_.Z , -MinVal);
    x := -v_.X / v_.Z * radius + centerx;
    y := ArcTan(-v_.X / v_.Z) / CPi * radius + centerx;
    result.X := trunc(convert(x,y,1));
    x :=  v_.Y / v_.Z * radius + centery;
    y := ArcTan( v_.Y / v_.Z) / CPi * radius + centery;
    result.Y := trunc(convert(x,y,1));
    }
    /////////////
end;

function TFNMath3D.GetRotOfMtrx(const M : TMatrix3D):TVec3D;
var
    v : TVec3D;
begin
	v := Norm(M.Row1.X,M.Row1.Y,0);
	result.z := ArcCos(v.X);
	if (v.y < 0) then
        Result.z := -Result.Z;
	//----------------------
	result.y := ArcSin(-m.Row1.z);
	//----------------------
	v := Vec3D( v.x * M.Row2.X + v.Y * M.Row2.Y ,
               -v.y * M.Row2.X + v.X * M.Row2.Y ,
                M.Row2.Z );
	result.x := ArcCos(Dot(v,Vec3D(0,1.0,0)));
end;
